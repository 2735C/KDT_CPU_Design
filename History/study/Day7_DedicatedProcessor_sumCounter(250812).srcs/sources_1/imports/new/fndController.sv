`timescale 1ns / 1ps

module fndController (
    input logic clk,
    input logic rst,
    input logic [13:0] number,
    output logic [3:0] fndCom,
    output logic [7:0] fndFont
);

    logic tick_1khz;
    logic [1:0] count;
    logic [3:0] digit_1, digit_10, digit_100, digit_1000, digit;

    // display 4자리에 번갈아 연결되는 속도
    clk_div_1khz U_Clk_Div_1khz (
        .clk      (clk),
        .rst      (rst),
        .tick_1khz(tick_1khz)
    );
    
    //display 4자리 중 하나를 선택하기 위한 2비트 카운터
    counter_2bit U_Counter (
        .clk  (clk),
        .rst  (rst),
        .tick (tick_1khz),
        .count(count)
    );

    // count에 따라 1110, 1101, 1011, 0111로 설정
    decoder_2x4 U_decoder_2x4 (
        .x(count),
        .y(fndCom)
    );

    //10진수를 각각 4자리로 분리
    disitSplitter U_DigitSplitter (
        .number    (number),
        .digit_1   (digit_1),
        .digit_10  (digit_10),
        .digit_100 (digit_100),
        .digit_1000(digit_1000)
    );

    // count, 즉, 연결되는 display 위치에 따라 출력되는 digit를 선택
    mux_4x1 U_mux_4x1 (
        .sel(count),
        .x0 (digit_1),
        .x1 (digit_10),
        .x2 (digit_100),
        .x3 (digit_1000),
        .y  (digit)
    );

    // 선택된 digit를 FND로 변환(7segment에 맞게)
    BCDtoFND_Decoder U_BCDtoFND (
        .bcd(digit),
        .fnd(fndFont)
    );

endmodule

module clk_div_1khz (
    input  logic clk,
    input  logic rst,
    output logic tick_1khz
);
    logic [$clog2(100_000):0] div_counter;

    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            div_counter <= 0;
            tick_1khz   <= 1'b0;
        end else begin
            if (div_counter == 100_000 - 1) begin
                div_counter <= 0;
                tick_1khz   <= 1'b1;
            end else begin
                div_counter <= div_counter + 1;
                tick_1khz   <= 1'b0;
            end

        end
    end

endmodule

module counter_2bit (
    input logic clk,
    input logic rst,
    input logic tick,
    output logic [1:0] count
);
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            count <= 0;
        end else begin
            if (tick) begin
                count <= count + 1;
            end
        end
    end
endmodule

module decoder_2x4 (
    input  logic [1:0] x,
    output logic [3:0] y
);
    always_comb begin
        y = 4'b1111;  // Default value
        case (x)
            2'b00: y = 4'b1110;
            2'b01: y = 4'b1101;
            2'b10: y = 4'b1011;
            2'b11: y = 4'b0111;
        endcase
    end
endmodule

module disitSplitter (
    input  logic [13:0] number,
    output logic [ 3:0] digit_1,
    output logic [ 3:0] digit_10,
    output logic [ 3:0] digit_100,
    output logic [ 3:0] digit_1000
);
    assign digit_1    = number % 10;
    assign digit_10   = (number / 10) % 10;
    assign digit_100  = (number / 100) % 10;
    assign digit_1000 = (number / 1000) % 10;

endmodule

module mux_4x1 (
    input  logic [1:0] sel,
    input  logic [3:0] x0,
    input  logic [3:0] x1,
    input  logic [3:0] x2,
    input  logic [3:0] x3,
    output logic [3:0] y
);
    always_comb begin
        y = 4'b0000;  // Default value
        case (sel)
            2'b00: y = x0;
            2'b01: y = x1;
            2'b10: y = x2;
            2'b11: y = x3;
        endcase
    end
endmodule

module BCDtoFND_Decoder (
    input  logic [3:0] bcd,
    output logic [7:0] fnd
);
    always_comb begin
        case (bcd)
            4'h0: fnd = 8'hc0;
            4'h1: fnd = 8'hf9;
            4'h2: fnd = 8'ha4;
            4'h3: fnd = 8'hb0;
            4'h4: fnd = 8'h99;
            4'h5: fnd = 8'h92;
            4'h6: fnd = 8'h82;
            4'h7: fnd = 8'hf8;
            4'h8: fnd = 8'h80;
            4'h9: fnd = 8'h90;
            4'ha: fnd = 8'h88;  // A
            4'hb: fnd = 8'h83;  // B
            4'hc: fnd = 8'hc6;  // C
            4'hd: fnd = 8'ha1;  // D
            4'he: fnd = 8'h86;  // E
            4'hf: fnd = 8'h8e;  // F      
            default: fnd = 8'hff;
        endcase
    end

endmodule
